...
aen_fc <- function(opt = "adam", lss = "mse", out_act = "linear"){
...
dec_output = dec_input %>% 
  layer_dense(units = 10, activation = "relu") %>% 
  layer_dense(units = input_size, activation = out_act) 
...
aen %>% compile(optimizer = opt, loss = lss)
...
anim_save(paste(opt,"_", lss,"_", out_act,".gif"), p)
}


aen_fc("adam", "mse", "gelu")

#anim_save("adam_mse_gelu_file.gif", p)
